...
public class DeleteHotelHandler implements HttpHandler {

    private static final Pattern pattern = Pattern.compile("/delete-hotel/(\\d+)");
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("DELETE".equalsIgnoreCase(exchange.getRequestMethod())) {
            try {
                boolean success = DatabaseUtil.deleteHotel(hotelId);
                JSONObject responseJson = new JSONObject()
                        .put("message", success ? "Xóa khách sạn thành công!" : "Không thể xóa khách sạn.");
